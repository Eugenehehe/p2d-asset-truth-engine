(() => {
  ...
  function build(){
    ...
    ds.rows=(state.sources.flow||[]).map((r,i)=>{
      ...
      const candidate=nr?parseDns(dns):'';
      return {include:!!candidate&&candidate!==current,line:'',serial,current_bin:current,dns,network_bin:candidate,to_warehouse:q('#directWarehouse').value||'UFIT',to_bin:candidate,row:i+2};
  ...
  function render(){
    ...
    q('#directTable').innerHTML=`<thead><tr><th><input id="directSelectAll" type="checkbox" checked></th><th>Serial</th><th>Flowtrac Bin</th><th>DNS</th><th>Network Bin</th><th>To Warehouse</th><th>To Bin</th></tr></thead><tbody>${ds.rows.map((x,i)=>`<tr><td><input type="checkbox" data-direct-include="${i}" ${x.include?'checked':''}></td><td class="code">${esc(x.serial)}</td><td>${esc(x.current_bin)}</td><td class="code">${esc(x.dns)}</td><td>${esc(x.network_bin)}</td><td><input data-direct-wh="${i}" value="${esc(x.to_warehouse)}"></td><td><input data-direct-bin="${i}" value="${esc(x.to_bin)}"></td></tr>`).join('')}</tbody>`;
  ...
  function templateCsv(){
    ...
    const rows=ds.rows.filter(x=>x.include);
    ...
    rows.forEach(x=>out.push(['',x.serial,x.to_warehouse,x.to_bin]));
    return out.map(r=>r.map(qv).join(',')).join('\r\n');
  }
  function exportTemplate(){
    const rows=ds.rows.filter(x=>x.include);if(!rows.length){alert('No rows selected.');return;}
